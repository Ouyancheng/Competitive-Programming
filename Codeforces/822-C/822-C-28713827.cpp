...
int n,x,di,ans,a,b,c;
int best[MAX_N];
pair<p32,p32> v[MAX_N*2];

int go() {
	repn(i,di) {
		int o = v[i].fi.fi, t = v[i].fi.se, l = v[i].se.fi, c = v[i].se.se;
		if(t) {
			best[l] = min(best[l],c);
		} else {
			if(x <= l || best[x-l] == inf32) continue;
			ans = min(ans,best[x-l]+c);
		}
	}
}
...
int main(void) {
	...
	cin >> n >> x;
	ans = inf32;
	repn(i,MAX_N) best[i] = inf32;
	repn(i,n) {
		cin >> a >> b >> c;
		v[di].fi.fi = a, v[di].fi.se = 0, v[di].se.fi = b-a+1, v[di++].se.se = c;
		v[di].fi.fi = b, v[di].fi.se = 1, v[di].se.fi = b-a+1, v[di++].se.se = c;
	}
	sort(v,v+di);
	go();
	if(ans == inf32) {
