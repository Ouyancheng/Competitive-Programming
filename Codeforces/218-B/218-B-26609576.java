...
public class Main {
	static TreeSet<Pair<Integer,Integer>> s = new TreeSet<>();
	static int arr[];
	static int n,m,hi = 0, lo = 0;
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < m; i++) {
			arr[i] = in.nextInt();
			s.add(new Pair<Integer,Integer>(arr[i],i));
		}
		Arrays.sort(arr);
		for(int i = 0; i < n; i++) {
			Pair<Integer,Integer> cur = s.last();
			hi += cur.x;
			s.remove(cur);
			cur.x -= 1;
			if(cur.x > 0) {
		...
		int taken = 0;
		for(int i = 0; i < m && taken < n; i++) {
			while(taken < n && arr[i] > 0) {
				lo += arr[i]--;
				taken++;
			}
		}
