...
int main(void) {
	...
	a = diff = min(cnt[1],min(cnt[2],cnt[4]));
	cnt[1] -= diff, cnt[2] -= diff, cnt[4] -= diff;

	b = diff = min(cnt[1],min(cnt[2],cnt[6]));
	cnt[1] -= diff, cnt[2] -= diff, cnt[6] -= diff;	

	c = diff = min(cnt[1],min(cnt[3],cnt[6]));
	cnt[1] -= diff, cnt[3] -= diff, cnt[6] -= diff;
	...
	repn(i,a) cout << 1 << " " << 2 << " " << 4 << breakl;
	repn(i,b) cout << 1 << " " << 2 << " " << 6 << breakl;
	repn(i,c) cout << 1 << " " << 3 << " " << 6 << breakl;
