...
#define MAX_N 40000001
#define MAX_R 300005

// variables for fenwick tree
int FT[MAX_N],n = MAX_N-1;

// user-defined variables
int power[32],s[MAX_R],n2,t,ans;

void calcPow(int a, int b) {
	for(int i = 0; i <= b; i++) {
		power[i] = 1<<i;
	}
}
...
int main(void) {
	...
	t = (int) floor(log2(s[n2])) + 1;
	repn(b,t) {
		int cnt = 0;
		repn(i,n2+1) {
			int cur = s[i] % power[b+1];
			if(cur-power[b] >= 0)
				cnt += queryFT((cur-power[b])+1);
			cnt += queryFT((cur+power[b])+1)-queryFT(cur+1);
			updateFT(cur+1,1);
		}
		if(cnt&1) ans |= (1<<b);
		repn(i,n2+1) updateFT(s[i]%power[b+1]+1,-1);
	}
	cout << ans << breakl;
	return 0;
}
